...
def s2xml(string):
    """convert python code to XML, return string stripping the tag <Module>"""
    result = py2xml(fromstring=string)
...
line 2""" '''
    ...
    def test_str_implicit_concat(self):
        string = "'part 1' ' /part 2'"
        assert s2xml(string) == \
            "<Expr><Str><s>'part 1'</s> <s>' /part 2'</s></Str></Expr>"

    def test_str_implicit_concat_line_continuation(self):
        string = r"""'part 1'  \
 ' /part 2'"""
        assert s2xml(string) == "<Expr><Str><s>'part 1'</s>  \\\n"\
            " <s>' /part 2'</s></Str></Expr>"

    def test_str_expr_concat(self):
        string = """('part 1'\n ' /part 2')"""
        assert s2xml(string) == \
            "<Expr>(<Str><s>'part 1'</s>\n"\
            " <s>' /part 2'</s></Str>)</Expr>"

    def test_str_expr_concat_with_comment(self):
        string = """('part 1' # ho \n ' /part 2')"""
        assert s2xml(string) == \
    ...
    def test_bytes_implicit_concat(self):
...
class TestExpressions:
    def test_expr_in_parenthesis(self):
        assert s2xml('(3 )') == '<Expr>(<Num>3</Num> )</Expr>'

    def test_tuple_in_parenthesis(self):
        assert s2xml('( 3 , 4  )') == \
            '<Expr>( <Tuple ctx="Load"><Num>3</Num> , <Num>4</Num>'\
            '</Tuple>  )</Expr>'

    def test_tuple_in_parenthesis_comma(self):
        assert s2xml('( 3 , 4 , )') == \
            '<Expr>( <Tuple ctx="Load"><Num>3</Num> , <Num>4</Num>'\
            ' ,</Tuple> )</Expr>'

    def test_expr_in_parenthesis2(self):
        assert s2xml('(  3 )') == '<Expr>(  <Num>3</Num> )</Expr>'

    def test_expr_in_parenthesis3(self):
        assert s2xml('(  3\n )') == '<Expr>(  <Num>3</Num>\n )</Expr>'

    def test_expr_in_parenthesis4(self):
        assert s2xml('(\n  3 )') == '<Expr>(\n  <Num>3</Num> )</Expr>'

    def test_expr_nl_comment(self):
        assert s2xml('(\n  3,\n # hi,\n4 )') == \
            '<Expr>(\n  <Tuple ctx="Load">'\
            '<Num>3</Num>,\n # hi,\n<Num>4</Num></Tuple> )</Expr>'

    def test_expr_semi_colon_separated(self):
        assert s2xml('3 ; 5') == \
            '<Expr><Num>3</Num></Expr>'\
            ' ; <Expr><Num>5</Num></Expr>'
...
class TestList:
    ...
    def test_list_multiline2(self):
        assert s2xml('[\n 1,\n  2,\n ]') == \
            '<Expr><List ctx="Load">[\n <Num>1</Num>'\
            ',\n  <Num>2</Num>,\n ]</List></Expr>'

    def test_list_multiline_comment(self):
        assert s2xml('[ 1, # hey\n  2,\n ]') == \
            '<Expr><List ctx="Load">[ <Num>1</Num>'\
            ', # hey\n  <Num>2</Num>,\n ]</List></Expr>'
...
class TestDict:
    ...
    def test_dict_multiline_comma(self):
        assert s2xml('{\n\n  "a": 2,\n"b" :3 \n, \n}') == \
            '<Expr><Dict>{\n\n  '\
            '<item><Str><s>"a"</s></Str>: <Num>2</Num></item>'\
            ',\n<item><Str><s>"b"</s></Str> :<Num>3</Num></item>'\
...
class TestAtrribute:
    ...
    def test_attr_par(self):
        assert s2xml('(foo).bar') == \
            '<Expr><Attribute ctx="Load">'\
            '<value>(<Name ctx="Load" name="foo">foo</Name>)</value>'\
            '.<attr>bar</attr></Attribute></Expr>'

    def test_attr_nl(self):
        assert s2xml('(foo\n .bar)') == \
            '<Expr>(<Attribute ctx="Load">'\
            '<value><Name ctx="Load" name="foo">foo</Name></value>'\
            '\n .<attr>bar</attr></Attribute>)</Expr>'

    def test_attr_space(self):
        assert s2xml('foo .bar') == \
            '<Expr><Attribute ctx="Load">'\
            '<value><Name ctx="Load" name="foo">foo</Name></value>'\
            ' .<attr>bar</attr></Attribute></Expr>'
...
class TestSubscript:
    ...
    def test_subscript_space_after(self):
        assert s2xml('foo[2] #') == \
            '<Expr><Subscript ctx="Load">'\
            '<value><Name ctx="Load" name="foo">foo</Name></value>'\
            '<slice>[<Index><Num>2</Num></Index>] #</slice>'\
    ...
    def test_slice_second_colon_without_step(self):
        assert s2xml('foo[1:: ]') == \
            '<Expr><Subscript ctx="Load">'\
            '<value><Name ctx="Load" name="foo">foo</Name></value>'\
            '<slice>[<Slice><lower><Num>1</Num></lower>::</Slice> ]</slice>'\
...
class TestBinOp:
    ...
    def test_binop_subtract(self):
        assert s2xml('1 - 2') == \
            '<Expr><BinOp><Num>1</Num><Sub> - </Sub><Num>2</Num></BinOp></Expr>'

    def test_expr_with_line_continuation(self):
        assert s2xml('''5 + \\\n  9''') == \
            '<Expr><BinOp><Num>5</Num><Add> + \\\n  </Add><Num>9</Num></BinOp></Expr>'

    def test_binop_multiline(self):
        assert s2xml('''(1 +\n 1)''') == \
            '<Expr>(<BinOp><Num>1</Num><Add> +\n </Add>'\
            '<Num>1</Num></BinOp>)</Expr>'

    def test_binop_multiline_xtreme(self):
        assert s2xml('''(1 \n\n+\n\n 1)''') == \
            '<Expr>(<BinOp><Num>1</Num><Add> \n\n+\n\n </Add>'\
            '<Num>1</Num></BinOp>)</Expr>'

    def test_expr_in_parenthesis_any(self):
        assert s2xml('( 2+ (3 )  )') == \
            '<Expr>( <BinOp><Num>2</Num><Add>+ </Add>(<Num>3</Num>'\
            ' )</BinOp>  )</Expr>'

    def test_expr_parenthesis_first_ele(self):
        assert s2xml('(1) + 2') == \
...
class TestBoolOp:
    ...
    def test_bool_op_more_than_2(self):
    ...
    def test_bool_op_multiline(self):
        assert s2xml('(1 and\n 2)') == \
            '<Expr>(<BoolOp op="And">'\
            '<value><Num>1</Num></value> and\n '\
            '<value><Num>2</Num></value>'\
...
class TestCompare:
    ...
    def test_compare_2_token_cmp(self):
...
class TestMultiline:
    ...
    def test_2line_end_with_op(self):
        assert s2xml('(4)\n(5)\n') == \
            '<Expr>(<Num>4</Num>)</Expr>\n'\
            '<Expr>(<Num>5</Num>)</Expr>\n'
...
class TestCall:
    ...
    def test_call_keyword_multiline(self):
        assert s2xml('foo( x =\n 2 )') == \
            '<Expr><Call><func><Name ctx="Load" name="foo">foo</Name></func>'\
            '( <keyword><arg>x</arg> =\n <value><Num>2</Num></value>'\
            '</keyword> )'\
    ...
    def test_call_arg_keyword(self):
    ...
    def test_call_paramenter_order(self):
    ...
    def test_call_keywords_before_after_starargs(self):
...
class TestListComp:
    ...
    def test_listcomp_multiline_on_if(self):
        assert s2xml('[a for a in b\n if a\n ]') == \
            '<Expr><ListComp>[<elt><Name ctx="Load" name="a">a</Name></elt>'\
            '<generators><comprehension> for '\
            '<target><Name ctx="Store" name="a">a</Name></target>'\
    ...
    def test_listcomp_multiline_on_in(self):
    ...
    def test_listcomp_multiline_on_for(self):
        assert s2xml('[a\n for a in b if a]') == \
            '<Expr><ListComp>[<elt><Name ctx="Load" name="a">a</Name></elt>'\
...
class TestGeneratorExp:
    ...
    def test_generatorexp_inside_call(self):
...
class TestFuncDef:
    ...
    def test_funcdef_args_default(self):
    ...
    def test_funcdef_args_defaults(self):
    ...
    def test_funcdef_comment(self):
        assert s2xml('def foo():\n    # comment\n    pass') == \
            '<FunctionDef name="foo">def foo<arguments>():</arguments>'\
            '<body>\n    # comment\n    <Pass>pass</Pass></body></FunctionDef>'

    def test_funcdef_after(self):
        assert s2xml('def foo():\n    pass\n8') == \
            '<FunctionDef name="foo">def foo<arguments>():</arguments>'\
            '<body>\n    <Pass>pass</Pass></body></FunctionDef>\n'\
            '<Expr><Num>8</Num></Expr>'

    def test_funcdef_vararg(self):
    ...
    def test_funcdef_kwonly_no_default(self):
        assert s2xml('def foo(*a, b ):\n    pass') == \
            '<FunctionDef name="foo">def foo'\
            '<arguments>('\
            '<vararg>*<arg name="a">a</arg></vararg>, '\
    ...
    def test_funcdef_kwonly_kwarg(self):
    ...
    def test_funcdef_kwonly_no_vararg(self):
    ...
    def test_funcdef_annotation(self):
...
class TestClassDef:
    ...
    def test_classdef_bases_multiline(self):
        assert s2xml('class Foo (\n Base,\n Base2) :\n    pass') == \
            '<ClassDef name="Foo">class Foo<arguments> (\n '\
            '<base><Name ctx="Load" name="Base">Base</Name></base>,\n '\
    ...
    def test_classdef_dcorator(self):
        assert s2xml('@deco\nclass Foo():\n    pass') == \
            '<ClassDef name="Foo">'\
    ...
    def test_classdef_no_args(self):
        assert s2xml('class Foo :\n    pass') == \
            '<ClassDef name="Foo">class Foo :'\
            '<body>\n    <Pass>pass</Pass></body></ClassDef>'

    def test_classdef_keyword(self):
    ...
    def test_classdef_all_args(self):
...
class TestAssign:
    ...
    def test_assign_space(self):
        assert s2xml('f  =   7') == \
            '<Assign><targets>'\
            '<Name ctx="Store" name="f">f</Name>  =   </targets>'\
            '<Num>7</Num></Assign>'

    def test_expr_semi_colon_separated2(self):
...
class TestImportFrom:
    ...
    def test_importfrom_dot_as(self):
    ...
    def test_importfrom_level(self):
    ...
    def test_importfrom_level2_module_none(self):
        assert s2xml('from .. import bar') == \
            '<ImportFrom level="2">from ..<module/> import <names>'\
            '<alias><name>bar</name></alias>'\
            '</names></ImportFrom>'

    def test_importfrom_par(self):
    ...
    def test_importfrom_par_nl(self):
    ...
    def test_importfrom_par_nl2(self):
...
class TestIf:
    ...
    def test_if_else_single_line(self):
...
class TestFor:
    ...
    def test_for_in_parenthesis_nl(self):
...
class TestBugs:
    def test_expr_parenthesis_complex(self):
    ...
    def test_elif_unicode_string(self):
